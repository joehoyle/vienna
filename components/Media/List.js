...
import { View, Image, StyleSheet, ScrollView, TouchableOpacity, Dimensions, TextInput } from 'react-native';
		...
		return (
			<View style={ styles.container }>
				{ this.props.posts.map( attachment => {
					return (
						<TouchableOpacity key={ attachment.id } onPress={ this.props.onEdit.bind( null, attachment ) } style={ styles.imageHighlight }>
							{ attachment.media_type === 'image' && attachment.media_details.sizes && attachment.media_details.sizes.thumbnail ? (
								...
								<Icon name="file" style={ { margin: imageSize - 60 } } size={ 60 } color="#9999" />
							) }
						</TouchableOpacity>
					);
				} ) }
